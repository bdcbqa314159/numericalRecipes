#include "../include/numRec_bits/svd.hpp"
#include <iostream>

SVD::SVD(matDoub_I &a):m(a.nrows()), n(a.ncols()), u(a), v(n,n), w(n){
...
matDoub SVD::range(double thresh = -1.){
	int i,j, nr=0;
...
void SVD::decompose(){
	...
	for (k=n-1; k>=0; k--){
		for (its=0; its<30; its++){
			...
			if (its == 29) throw std::string("no convergence in 3- svdcmp iterations");
			x=w[l];
			nm=k-1;
			y=w[nm];
			g=rv1[nm];
...
matDoub SVD::nullspace(double thresh){// = -1.){
...
void SVD::solve(vecDoub_I &b, vecDoub_O &x, double thresh){ //} = -1.){

	int i, j, jj;
	double s;

	if (b.size() != m | x.size() != n) throw std::string{"Singular matrix in SVD: solve bad sizes"};
	vecDoub tmp(n);
	tsh = (thresh >= 0. ? thresh : 0.5*sqrt(m+n+1.)*w[0]*eps);
	for (j = 0; j<n ; j++){
		s = 0.0;
...
void SVD::solve(matDoub_I &b, matDoub_O &x, double thresh){//} = -1.){

	int i, j, m = b.ncols();
	if (b.nrows() != n || x.nrows() != n || b.ncols() != x.ncols()){
		throw std::string{"SVD::solve bad sizes"};
	}
	vecDoub xx(n);
	for (j=0; j<m; j++){
