#include "../include/numRec_bits/qrdcmp.hpp"
...
: n(a.nrows()), qt(n,n), r(a), sing(false){
	...
	for (k = 0; k<n-1; k++){
		...
		if (scale == 0.0){
			...
			for (sum=0.0, i=k; i<n; i++) {sum += r(i,k)*r(i,k);}
...
void QRdcmp::rsolve(vecDoub_I &b, vecDoub_O &x){
	...
	if (sing) throw std::string{"attempting solve in a singular QR"};
...
void QRdcmp::update(vecDoub_I &u, vecDoub_I &v){
	...
	for (i=k-1; i>=0; i--){
		rotate(i,w[i],-w[i+1]);
		if (w[i]==0.0) w[i] = abs(w[i+1]);
		else if (abs(w[i])>abs(w[i+1])) w[i] = abs(w[i])*sqrt(1.0+ pow(w[i+1]/w[i], 2));
		else w[i] = abs(w[i+1])*sqrt(1.0+ pow(w[i]/w[i+1], 2));
	}

	for (i=0; i<n; i++) r(0,i) += w[0]*w[i];
	for (i=0; i<k; i++) rotate(i,r(i,i), -r(i+1,i));
